...
if (isset($_GET['action'])) {
    ...
    if (isset($_SESSION['shopping_cart'][$id])) {
        ...
        echo json_encode([
            ...
            'new_quantity' => $_SESSION['shopping_cart'][$id]['quantity']
        ]);
        exit();
    } else {
    ...
    fetch(`shoppingCart.php?action=remove&id=${id}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                if (totalAmount) {
                    totalAmount.textContent = 'Rp ' + new Intl.NumberFormat().format(data.new_total);
                }
                ...
                alert('Error: ' + (data.message || 'Unable to remove item.'));
            }
        })
        .catch(error => {
